...
/* ------------------------------------------------------------------ */
/*  Persistent storage helpers (safe / optional)                       */
/* ------------------------------------------------------------------ */
const store = {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setFirm(savedFirm);
      setEmployees(emp && emp.length ? emp : seedEmployees);
      setTrips(trp && trp.length ? trp : seedTrips);
      setTrains(trn && trn.length ? trn : seedTrains);
      if (rt) setRates(rt);
      if (mw && mw.length) setMinWages(mw);
      setPenalties(pen && pen.length ? pen : seedPenalties);
    ...
    >
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6 py-5">
        ...
        <footer className="text-center mt-8 mb-2 text-[11px]" style={{ color: T.slateSoft }}>
          Prototype • data saved on this device • demo figures shown
        </footer>
      </div>
    </div>
  );
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl p-6" style={{ background: T.card, border: `1px solid ${T.line}` }}>
          ...
          <p className="text-[11px] text-center mt-3" style={{ color: T.slateSoft }}>
            Prototype • sample staff and trips are pre-loaded to explore
          </p>
        </div>
      </div>
    </div>
